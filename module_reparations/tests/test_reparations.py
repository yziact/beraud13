...
class TestsReparations(common.TestsCommon):
    ...
    def setUp(self):
        ...
        self.atom_user = self.UserObj.search([('name','ilike','Nicolas Clair')])
        self.atom_user.company_id = 3

        """
        Users.create({
            #'company_id' : 3,
            'name': 'Jean Reparations Atom',
            'login': 'jeanratom',
            'alias_name': 'jeanratom',
            'email': 'jeanratom@example.com',
            'groups_id': [(6, 0, [user_group_employee.id, groupe_technique.id])]})
        """

        self.atom_tech = self.UserObj.search([('name','ilike','Brice Roudier')])
        self.atom_tech.company_id = 3
    ...
    def test01_repair_with_tech(self):
        ...
        self.assertEqual(repair_id.location_dest_id.name, 'Customers')
        self.assertEqual(repair_id.invoice_method, 'after_repair')
        self.assertEqual(repair_id.tech.name, 'Brice Roudier')

        line_one = self.RepairLineObj.create({
            'repair_id': repair_id.id,
            'name': repair_id.name,
            #'location_id': self.ber_loc_id.id,
            #'location_dest_id': self.customer_loc_id.id,
        ...
        line_two = self.RepairLineObj.create({
            'repair_id': repair_id.id,
            'name': repair_id.name,
            #'location_id': self.ber_loc_id.id,
            #'location_dest_id': self.customer_loc_id.id,
        ...
        # adding a line when repair is in state 'confirmed'
        line_added = self.RepairLineObj.create({
            'repair_id': repair_id.id,
            'name': repair_id.name,
            #'location_id': self.ber_loc_id.id,
            #'location_dest_id': self.customer_loc_id.id,
        ...
        #print "line_added.location_id.name : %s" % line_added.location_id.name
        #print "line_added.location_dest_id.name : ", line_added.location_dest_id.name
        ...
        # before ending the repair, remove a line. We'll do another repair after this one, and we'll check if the 
        # isSale flag sets correctly when the tech has a product used in his stock, and uses it for another partner that has a company_id
        # that is different
        line_one.unlink()
        ...
        #################### SECOND REPAIR ######################
        #########################################################
        # Create new repair, this time for a partner belonging to the OTHER COMPANY 
        # than the partner in the first repair. With the same tech, so his stock is the one
        # that was filled with the BL of the previous repair
        repair_id = self.RepairObj.create({
            'partner_id': self.partner_atom_id.id,
            'product_id': self.productA.id,
            'product_uom': self.productA.uom_id.id,
            'tech': self.atom_tech.id,
            'company_id': 3,
        ...
        # tech should have productB still in his stock cause it was NOT USED in the first repair.
        line_one = self.RepairLineObj.create({
            'repair_id': repair_id.id,
            'name': repair_id.name,
            #'location_id': self.ber_loc_id.id,
        ...
        # confirm the repair
        repair_id.sudo(self.atom_user.id).action_confirm()
        ...
        # end the repair now
        repair_id.sudo(self.atom_user.id).action_repair_end()

        print "looking for moves generated by the end of the repair"
        moves_generated = self.MoveObj.search([('origin','ilike', repair_id.name)])
        moves_generated = [m for m in moves_generated if m not in repair_id.bl_internal.move_lines]
        print "moves_generated : ", moves_generated

        # make sure that none of the moves are isSale
        for move in moves_generated:
            if move.product_id.id == self.productB.id:
                # this move should be an isSale
                self.assertEqual(move.isSale, True, msg='move.isSale is not True : %s' % move.isSale)
            if move.product_id.id == self.productC.id:
                # this move should NOT be an isSale
                self.assertEqual(move.isSale, False, msg='move.isSale is not False : %s' % move.isSale)
