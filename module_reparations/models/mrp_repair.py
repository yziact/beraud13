...
class MrpRepairInh(models.Model):
    ...
    # moves generated by action confirm, our site repair case
    moves_from_confirm = fields.Many2many('stock.move')

    # moves added while repair was ongoing, created when a line is created
    moves_added_meanwhile = fields.Many2many('stock.move')
...
### BERAUD SITE REPAIR CONFIRM CASE ###
            if not repair.clientsite:
                ...
                for line in repair.operations :
                    ...
                    repair_obj.write(cr, uid, [repair.id], {'moves_from_confirm' : [(4, move_id)]} )

                print "*** MOVES FROM CONFIRM : ", repair.moves_from_confirm
...
### BERAUD SITE REPAIR DONE CASE ###
            if not repair.clientsite:
                ...
                # take moves from moves_from_confirm, and make them done.
                for move in repair.moves_from_confirm :
                    move_obj.action_done(cr, uid, move.id)

                for move in repair.moves_added_meanwhile :
                    move_obj.action_done(cr, uid, move.id)
...
class MrpRepairLine(models.Model):
    ...
    def create(self, vals):
        ...
        print "[mrp_repair_line create] vals are : "
        pp.pprint(vals)
        ...
        print "[mrp_repair_line create] vals (updated) are : "
        pp.pprint(vals)
        ...
        move_id = move_obj.create({
            ...
            'product_uom_qty': abs(repair_line.product_uom_qty),
        ...
        print "repair.moves_added_meanwhile are now : ", repair_line.repair_id.moves_added_meanwhile
    ...
    def unlink(self):
        ...
        for repair_line in self:
            if repair_line.associated_move :
                # first we remove the move from repair_id.moves_added_meanwhile
                for move in repair_line.repair_id.moves_added_meanwhile:
                    if move.id == repair_line.associated_move.id : 
                        print "repair_id.moves_added_meanwhile before ", repair_line.repair_id.moves_added_meanwhile
                        repair_line.repair_id.write({'moves_added_meanwhile' : [(3, move.id)] })
                        print "repair_id.moves_added_meanwhile after ", repair_line.repair_id.moves_added_meanwhile
