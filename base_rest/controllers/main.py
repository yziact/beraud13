    ...
    def _add_default_methods(cls, bases, members):
        if "RestController" in globals() and RestController in bases:
            @route([
                ...
                '<string:_service_name>/get',
                '<string:_service_name>/get/<int:_id>',
            ], methods=['GET'])
            def get(self, _service_name, _id=None, **params):
                return self._process_method(_service_name, 'get', _id, params)

            @route([
                '<string:_service_name>/get_inactive',
            ], methods=['GET'])
            def get_inactive(self, _service_name, **params):
                return self._process_method(_service_name, 'get_inactive', params=params)

            @route([
                '<string:_service_name>/search',
            ], methods=['GET'])
            def search(self, _service_name, **params):
                return self._process_method(_service_name, 'search', params=params)

            @route([
                '<string:_service_name>/create',
            ], methods=['POST'])
            def create(self, _service_name, **params):
                return self._process_method(_service_name, 'create', params=params)

            @route([
                '<string:_service_name>/update/<int:_id>',
            ], methods=['PUT'])
            def update(self, _service_name, _id, **params):
                return self._process_method(_service_name, 'update', _id, params)

            @route([
                '<string:_service_name>/delete/<int:_id>',
            ], methods=['DELETE'])
            def delete(self, _service_name, _id):
                return self._process_method(_service_name, 'delete', _id)

            @route([
                '<string:_service_name>/custom',
                '<string:_service_name>/custom/<int:_id>'
            ], methods=['POST'])
            def custom(self, _service_name, _id=None, **params):
                return self._process_method(_service_name, 'custom', _id, params)
...
class RestController(Controller, metaclass=RestControllerType):
    ...
    def service_component(self, service_name):
        ...
        with self.work_on_component() as work:
            service = work.component(usage='abstract')
            yield service
    ...
    def _process_method(self, service_name, method_name, _id=None, params=None):
        ...
        with self.service_component('abstract') as service:
            result = service.dispatch(service_name, method_name, _id, params)
