...
class BaseRestService(AbstractComponent):
    ...
    def _get_validator(self, validator_method, **model):
        if not hasattr(self, validator_method):
            return None
        v = getattr(self, validator_method)(**model)
        if isinstance(v, dict):
            return Validator(v, purge_unknown=True)
        return v

    def _get_input_validator(self, method_name, **model):
        return self._get_validator('_validator_%s' % method_name, **model)

    def _get_output_validator(self, method_name, **model):
        return self._get_validator('_validator_return_%s' % method_name, **model)
    ...
    def _secure_input(self, method, params, **model):
        """
        ...
        if hasattr(method, "skip_secure_params"):
            return params
        v = self._get_input_validator(method_name, **model)
        if v is None:
    ...
    def _secure_output(self, method, response, **model):
        """
        ...
        if hasattr(method, "skip_secure_response"):
            return response
        v = self._get_output_validator(method_name, **model)
        if not v:
            _logger.warning(
    ...
    def dispatch(self, service_name, method_name, _id=None, params=None):
        """
        ...
        secure_params = self._secure_input(func, params, **{'model': service_name})
        if _id:
            secure_params['_id'] = _id
        secure_params['_model'] = service_name
        res = func(**secure_params)
        self._log_call(func, params, secure_params, res)
        return self._secure_output(func, res, **{'model': service_name})

    def _validator_delete(self, **model):
        """
